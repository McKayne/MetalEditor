...
class SplineController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Spline"
    }
    
    func popToScene(sender: UIBarButtonItem) {
        
        let points: [Float] = [0, 1, 2, 3, 4]
        let f: [Float] = [0, 2, 1, 4, 0]
        let spline = Spline(points: points, f: f)
        
        //curve.shiftTo(xShift: 0, yShift: 0, zShift: 0.5)
        
        RootViewController.scenes[RootViewController.currentScene].appendObjectWithoutUpdate(object: spline)
